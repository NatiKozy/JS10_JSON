...
document.addEventListener('DOMContentLoaded',()=>{
    const heroesLocal = localStorage.getItem('heroPost');
    if(!heroesLocal) {
        localStorage.setItem('heroPost', JSON.stringify(arr))
    }
    createListPage(JSON.parse(heroesLocal));
})
...
function createTemplate(hero) {
    ...
    <h3>Рейтинг</h3>
    <h4>${rating}</h4>
    <select onChange = 'getValueSelect(event)' id= ${id} class='select'>
    <option hidden>Поставьте рейтинг</option>
...
function getValueSelect(event){
    ...
    localStorage.setItem('heroPost', JSON.stringify(newArr));
